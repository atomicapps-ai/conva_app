...
- Small, focused branches, cut from `dev` and PR'd into `dev` — that is the
  target in all three repos; `main` is release-only. Don't commit to `main`/`dev`
  locally. Commit/push only when asked. Keep the IPC Rust↔TS mirror in lockstep
