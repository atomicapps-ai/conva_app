...
export function StartListeningButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      className={[
        "inline-flex shrink-0 items-center justify-center gap-2.5 rounded-lg border-0 text-sm font-bold text-[#EEF1FF]",
        ...
        compact ? "h-11 w-11" : "px-5 py-[13px]",
        className,
      ].join(" ")}
    >
      <LockedIcon name="action-start-listening" size={18} />
      {!compact && label}
    </button>
  );
}
