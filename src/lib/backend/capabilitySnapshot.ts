...
export function webOperations(): OperationAvailability {
  return {
    ...
    // Web goes through the session BFF (webAuth.ts); desktop's own path is
    // avatar.rs — see the platform design doc referenced on ConvaBackend.auth.
    ...
    "context.storeDocFile": unsupported("Writes into the local Context folder — desktop only; on web a dropped or pasted file goes to the hosted library via rag.upload."),
    ...
    "context.toggleFavoritePersona": unimplemented(M1),
